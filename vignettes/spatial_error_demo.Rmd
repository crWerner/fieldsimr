...
We conceive a scenario in which 100 maize hybrids are measured for grain yield (t/ha) and plant height (cm) in three locations. The first two locations include three blocks, and the third location includes two blocks (i.e., we lost one replicate). Each replicate comprises 10 rows and 10 columns. The blocks are arranged in row direction (“above-and-below”). The plot length (column direction) is 4 meters, and the plot width (row direction) is 2 meters. 

```{r}
n_envs <- 3 # Number of environments.
n_traits <- 2 # Number of traits.
n_blocks <- c(3, 3, 2) # Number of blocks in the three environments, respectively.
block_dir <- "row" # Layout of blocks (“above-and-below”).
