...
**Note:** ['FieldSimR’](https://CRAN.R-project.org/package=fieldSimR) also provides wrapper functions to enable simulation of genetic values using unstructured model for genotype-by-environment (GxE) interaction. This is demonstrated in FieldSimR::unstructured_GxE_demo.
...
```{r}
cor_A <- matrix(
  c( # Matrix of additive genetic correlations grain yield and plant height.
    1.0, 0.5,
    0.5, 1.0
  ),
  ncol = 2
...
```{r}
mean_DD <- c(0.4, 0.1) # c(grain yield, plant height)

var_DD <- c(0.1, 0.1) # c(grain yield, plant height)
