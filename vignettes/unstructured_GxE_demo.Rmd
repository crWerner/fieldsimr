...
**Note:** ['FieldSimR’](https://CRAN.R-project.org/package=fieldSimR) also provides wrapper functions to enable simulation of genetic values using a compound symmetry model for GxE interaction. This is demonstrated in FieldSimR::compound_symmetry_GxE_demo.

The **core function of ['FieldSimR’](https://CRAN.R-project.org/package=fieldSimR)** generates plot errors comprising 1) a spatially correlated error term, 2) a random error term, and 3) an extraneous error term. Spatially correlated errors are simulated using either bivariate interpolation, or a two-dimensional autoregressive process of order one (AR1:AR1). Through the combination of the plot errors with (simulated) genetic values, ['FieldSimR’](https://CRAN.R-project.org/package=fieldSimR) enables simulation of multi-environment plant breeding trials at the plot. This is demonstrated in FieldSimR::spatial_error_bivar_demo.
...
To provide a comprehensive overview of the unstructured modelling approach for GxE interaction, we assume additive and dominance gene action for both grain yield and plant height. Details on how ['AlphaSimR’](https://CRAN.R-project.org/package=AlphaSimR) simulates additive and non-additive biological effects can be found in the ["Traits in AlphaSimR"](https://cran.r-project.org/web/packages/AlphaSimR/vignettes/traits.pdf) vignette.
...
We assume similar dominance degrees and dominance degree variances in all three environments for the two traits, respectively. We can achieve that by providing a vector with only two values. The two traits will receive the same value in each environment if the length of the two vectors is equal to `n_traits.` 
...
```{r}
T_cor_A <- matrix(
  c( # Matrix of additive genetic correlations between the two traits.
    1.0, 0.6,
    0.6, 1.0
  ),
  ncol = 2
